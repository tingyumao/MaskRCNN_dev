...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        y1, x1, y2, x2 = tf.split(boxes, 4, axis=2) # batch, num_box, 1
        h = y2 - y1
        w = x2 - x1
        ...
        roi_level = tf.squeeze(roi_level, 2) # batch, nbox

        # Loop through levels and apply ROI pooling to each. P2 to P5.
        pooled = []
        box_to_level = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.where(tf.equal(roi_level, level)) # batch*nbox, 2 input.axis_dim
            level_boxes = tf.gather_nd(boxes, ix) # batch*nbox, 4

            # Box indicies for crop_and_resize.
            box_indices = tf.cast(ix[:, 0], tf.int32) # batch*nbox
            ...
            pooled.append(tf.image.crop_and_resize(
                ...
                method="bilinear")) # batch*nbox, poolh 7, poolw 7, inchannel

        # Pack pooled features into one tensor
        pooled = tf.concat(pooled, axis=0) # batch*all_box, 7, 7, inchannel

        # Pack box_to_level mapping into one array and add another
        # column representing the order of pooled boxes
        box_to_level = tf.concat(box_to_level, axis=0) # batch*all_nox, 2
        # batch*all_nox,
        box_range = tf.expand_dims(tf.range(tf.shape(box_to_level)[0]), 1)
        box_to_level = tf.concat([tf.cast(box_to_level, tf.int32), box_range],
                                 axis=1) # batch*all_nox, 3 (xi,yi,i)
        ...
        ix = tf.nn.top_k(sorting_tensor, k=tf.shape(
            box_to_level)[0]).indices[::-1] # from small to large, 
        ix = tf.gather(box_to_level[:, 2], ix)
        pooled = tf.gather(pooled, ix) # rearrange pooled, batch*all, 7, 7, inchannel

        # Re-add the batch dimension
        pooled = tf.expand_dims(pooled, 0) # 1, batch*all, 7, 7, inchannel
        return pooled
...
def build_fpn_mask_graph(rois, feature_maps,
    ...
    """
    # ROI Pooling: use timedistributed to apply Conv2D to each box
...
# why do we need to generate random rois?
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        layer_regex = {
            ...
            "heads": r"(mrcnn\_class\_.*)|(mrcnn\_bbox\_.*)|(rpn\_.*)|(fpn\_.*)",
    ...
    def ancestor(self, tensor, name, checked=None):
        ...
        for p in parents:
            ...
            if bool(re.fullmatch(name, p.name)):
                return p
            checked.append(p)
            a = self.ancestor(p, name, checked) # recursive searching
            if a is not None:
                return a
        return None
